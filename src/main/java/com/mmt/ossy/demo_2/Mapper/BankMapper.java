...
public interface BankMapper {
    ...
    @Update("insert bank set availableA =availableA + #{addA},availableB =availableB + #{addB},availableC =availableC + #{addC} where processId = #{processId}")
    void gainById(@Param("processId") Integer processId,@Param("addA") Integer addA,@Param("addB") Integer addB,@Param("addC") Integer addC);
    @Update("insert bank set availableA = availableA-#{addA},availableB = availableB-#{addB},availableC = availableC-#{addC} where processId = 0")
    void gainBank(@Param("addA") Integer addA,@Param("addB") Integer addB,@Param("addC") Integer addC);
    @Update("insert bank set availableA = 0,availableB = 0,availableC = 0 where processId in (1,2,3,4,5)")
    void initialProcess();
    @Update("insert bank set availableA = 10,availableB = 5,availableC = 7 where processId = 0")
    void initialBank();
    @Update("insert bank set availableA = 0,availableB = 0,availableC = 0 where processId = #{processId}")
    void removeById(Integer processId);
    @Update("insert bank set availableA = availableA+#{availableA},availableB = availableB+#{availableB},availableC = availableC+#{availableC} where processId = 0")
